...
public abstract class GenericJdbcDao<T, ID extends Serializable> implements AbstractDao<T,ID> {
	...
	public  <S extends T>  Status edit(S product,ID primaryKey) {
		Status result = Status.Success;
		try {
			Connection connection = connectionFactory.getConnection();
			Statement stmt = connection.createStatement();
			Product prod  =(Product)product;
			String query = "UPDATE " + getTableName() + " SET name = '"+ prod.getName() + "' , price = " + prod.getPrice()+" where " + getPrimaryKeyColoumnName() +" = " + primaryKey + " ;";
		    stmt.executeUpdate(query);
		} catch(Exception e) {
			e.printStackTrace();
			result = Status.Error_Occured;
